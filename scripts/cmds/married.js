module.exports = {
  config: {
    ...
    longDescription: "",
    category: "love",
    guide: "{pn} @mention1 @mention2 or reply to someone"
  },
  ...
  onStart: async function ({ event, api, args }) {
    ...
    // যদি দুজন মানুষকে মেনশন করা হয়
    if (mention.length >= 2) {
      one = mention[0];
      two = mention[1];
    } 
    // অথবা যদি একজনকে মেনশন করা হয় এবং অন্য কারো মেসেজে রিপ্লাই দেওয়া হয়
    else if (mention.length === 1 && messageReply) {
      one = mention[0];
      two = messageReply.senderID;
    } 
    else {
      return api.sendMessage("Please tag or mention two people! (Example: /married @person1 @person2 or tag one and reply to another)", threadID, messageID);
    }
